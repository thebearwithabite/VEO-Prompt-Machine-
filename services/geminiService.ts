...
// Add ingestScriptFile to support document text extraction
/**
 * Ingests a script file (PDF, DOCX, etc.) using Gemini to extract text.
 */
export const ingestScriptFile = async (base64: string, mimeType: string): Promise<string> => {
    const ai = getAiClient();
    const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: {
            parts: [
                { text: "Extract and return the full text script from this document. Maintain screenplay formatting if possible." },
...
export const extractAssetsFromScript = async (script: string) => {
    ...
    const response = await ai.models.generateContent({
        ...
        contents: script,
        config: {
            systemInstruction: "Read the script and extract all major Characters, Locations, and Props. Be specific and detailed in descriptions. Return a JSON array of objects.",
            responseMimeType: 'application/json',
            responseSchema: {
              type: Type.ARRAY,
              items: {
                type: Type.OBJECT,
                properties: {
                  name: { type: Type.STRING, description: "Name of the entity (e.g. 'John Doe' or 'The Haunted Kitchen')" },
                  description: { type: Type.STRING, description: "Detailed visual description of the entity as implied by the script." },
                  type: { type: Type.STRING, description: "Must be one of: 'character', 'location', 'prop', 'style'" }
                },
